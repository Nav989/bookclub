...
const emailSend= async(req,res)=>{
    ...
    if (findUser){
        ...
        if(otpData){
            ...
            mailer(otpData.email,otpData.otp);
        }else{
...
const addbook = async(req,res)=>{
    ...
    const {book_image,book_name,author_name,availability_status,page_count,description,publish_Date,quantity} = req.body
    ...
    if(!book_image || !book_name || !author_name || !availability_status || !page_count || !description || !publish_Date || !quantity){
        ...
        if(bookData){
            res.status(200).json({success:bookData});
        }else{

            res.status(400).json({message:"not added books"})
...
const editbook = async(req,res)=>{
    ...
    const bookData = await book.update({ book_image:book_image,
                                      book_name:book_name ,
                     availability_status:availability_status ,
                                        page_count:page_count,
                                        description:description,
   ...
   const bookData = await book.findAll({attributes:[ 'book_image','book_name','author_name','quantity'],where:{author_name:req.body.author_name}});
