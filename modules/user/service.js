...
const allborrowedbook = async (req, res) => {
    ...
    if (user_data) {
        ...
        res.status(400).json({
            message: "user doesn't have any book borrow"
        })
    }
...
const returnbook = async (req, res) => {
    ...
    if (book_data.quantity < 10 && book_data.availability_status == "yes" && user_data.borrowbook_qty - 1 <= 3) {
        ...
        const returnData = await books.update({
            return_book: req.body.return_book,
...
const recommendbook = async (req, res) => {
    ...
    if (user_data || book_data) {
        ...
        if (sendmail) {
            ...
            const message = ("THIS YOUR MY FAVORITE BOOK RECOMMED YOU :-" + book_data.book_name + "\n" + " BOOK LINK:-" + "localhost:5000/book/details");
...
const ratebook = async (req, res) => {
    ...
    if (user_data && book_data) {
        ...
        if (ratingData) {
            ...
            res.status(400).json({
                message: "cant able to rate book"
            })
        }

    } else {
        res.status(400).json({
            message: "fill user and book details"
        })
    }
...
const viewpending = async (req, res) => {
    ...
    if (user_data && book_data) {
        ...
        if (!viewdays) {
            ...
            res.status(200).json({
                success: viewdays + " pending days " + day
            });
        }
...
const favbook = async (req, res) => {
    ...
    if (user_data || book_data) {
        ...
        if (favData) {
            ...
            res.status(400).json({
                message: "cant able to add fav book"
            })
        }
